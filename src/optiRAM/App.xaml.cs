...
{
    internal const string SingleInstanceMutexName = "optiRAM_SingleInstance_B7F3A2";
    internal const string ActivationSignalName = "optiRAM_Activate_B7F3A2";
    ...
    protected override async void OnStartup(StartupEventArgs e)
    {
        ...
        DispatcherUnhandledException += (s, args) =>
        {
            MessageBox.Show(
                $"Unhandled error:\n\n{args.Exception}",
                "optiRAM Error", MessageBoxButton.OK, MessageBoxImage.Error);
            args.Handled = true;
        };
        ...
        if (!createdNew)
        {
            ...
            {
                MessageBox.Show(
                    "optiRAM is already running, but the existing instance could not be activated.\n\nRestore it from the system tray.",
                    "optiRAM",
                    MessageBoxButton.OK,
        ...
        if (!IsRunningAsAdmin())
        {
            ...
            var result = MessageBox.Show(
                "optiRAM requires administrator privileges to manage system memory.\n\n" +
                "Yes = Restart as administrator\n" +
                "No = Continue in read-only mode (monitoring only)",
                "optiRAM", MessageBoxButton.YesNoCancel, MessageBoxImage.Warning);
    ...
    {
        var hwnd = NativeMethods.FindWindow(null, "optiRAM");
        if (hwnd == IntPtr.Zero)
            return false;
