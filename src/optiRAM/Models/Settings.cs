...
public class Settings
{
    ...
    private static readonly string SettingsDir = Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "optiRAM");
    ...
    public static Settings Load()
    {
        try
        {
            // Migrate settings from the old "RAMSpeed" directory if they exist
            MigrateOldSettings();
    ...
    /// <summary>One-time migration: copy settings from the old "RAMSpeed" directory if the new one doesn't exist yet.</summary>
    private static void MigrateOldSettings()
    {
        if (File.Exists(SettingsFile)) return; // already have new settings
