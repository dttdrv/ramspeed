...
public class TaskSchedulerHelperTests
{
    ...
    public void BuildTaskXml_includes_working_directory()
    {
        ...
        Assert.Equal(@"C:\Program Files\RAMSpeed", document.Root?.Element(ns + "Actions")?.Element(ns + "Exec")?.Element(ns + "WorkingDirectory")?.Value);
    }
    ...
    public void BuildTaskXml_includes_author_in_registration_info()
    {
        ...
        Assert.Equal("RAMSpeed", document.Root?.Element(ns + "RegistrationInfo")?.Element(ns + "Author")?.Value);
    }
